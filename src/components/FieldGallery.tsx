  ...
  return (
    <section className="py-24 px-4 sm:px-8 bg-dk-bg relative overflow-hidden border-t border-dk-border">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-dk-border pb-8">
          <div>
            <div className="inline-flex items-center gap-2 font-sans text-xs text-hl-moss uppercase tracking-wider font-semibold mb-3">
              <Camera className="w-4 h-4 text-hl-moss" />
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
          {items.map((item, index) => (
            <div
              ...
              className={`relative group rounded-lg overflow-hidden border border-dk-border/40 bg-dk-secondary cursor-pointer transition-all duration-300 hover:border-hl-terracotta ${item.aspect}`}
            >
              <img
                src={item.src}
                alt={item.title}
              ...
              />
              <div className="absolute inset-0 bg-gradient-to-t from-dk-bg/95 via-transparent to-transparent opacity-60 group-hover:opacity-90 transition-opacity"></div>

              {/* Technical Stamp Overlay */}
              <div className="absolute top-3 left-3 font-sans text-[10px] text-dk-text bg-dk-bg/85 px-2.5 py-0.5 border border-dk-border/40 rounded-full tracking-wider font-semibold">
                SHOT #{item.id}
              </div>

              <div className="absolute bottom-3 left-3 right-3 font-sans text-xs text-dk-text flex items-center justify-between">
                <div>
                  <span className="block font-bold text-[11px] truncate max-w-[200px] sm:max-w-[240px]">
                    {item.title}
                  </span>
                  <span className="text-[10px] text-dk-secondary-text">{item.location}</span>
                </div>
                <Maximize2 className="w-4 h-4 text-hl-terracotta opacity-0 group-hover:opacity-100 transition-opacity shrink-0" />
              </div>
            </div>
          ))}
        </div>
      ...
      {selectedImageIndex !== null && (
        <div className="fixed inset-0 z-50 bg-dk-bg/95 backdrop-blur-md flex items-center justify-center p-4 sm:p-8">
          <button
            onClick={closeLightbox}
            className="absolute top-6 right-6 p-3 bg-dk-secondary border border-dk-border text-dk-text hover:text-hl-terracotta rounded-full shadow-md z-10"
          >
            <X className="w-6 h-6" />
          </button>
          ...
          <button
            onClick={prevImage}
            className="absolute left-4 sm:left-8 p-3 bg-dk-secondary border border-dk-border text-dk-text hover:text-hl-terracotta rounded-full shadow-md z-10"
          >
          ...
          <button
            onClick={nextImage}
            className="absolute right-4 sm:right-8 p-3 bg-dk-secondary border border-dk-border text-dk-text hover:text-hl-terracotta rounded-full shadow-md z-10"
          >
            <ChevronRight className="w-6 h-6" />
          </button>

          <div className="max-w-4xl w-full flex flex-col items-center space-y-4">
            <div className="max-h-[75vh] w-full rounded-lg overflow-hidden border border-dk-border bg-dk-bg">
              <img
            ...
            <div className="w-full font-sans text-xs text-dk-text bg-dk-secondary p-4 border border-dk-border rounded-full px-6 flex items-center justify-between">
              <div>
                <span className="text-hl-terracotta font-bold block">
                  SHOT #{items[selectedImageIndex].id} · {items[selectedImageIndex].title}
                </span>
                <span className="text-dk-secondary-text text-[11px]">
                  {items[selectedImageIndex].location} · Refúgio Santalena
                </span>
              </div>
              <span className="text-hl-moss font-semibold">
